...
void loop() {
  ...
  if (t_actual - ultimoMuestreo >= INTERVALO_MS) {
    ...
    if (abs(a.acceleration.x) < 40.0 && abs(a.acceleration.y) < 40.0 && abs(a.acceleration.z) < 40.0) {
      ...
      // Enviar trama CSV estandarizada: T, Ax, Ay, Az, EMG
      udp.print(t_actual - tiempoInicio); udp.print(",");
      udp.print(axFiltro, 2); udp.print(",");
      udp.print(ayFiltro, 2); udp.print(",");
      udp.print(azFiltro, 2); udp.print(",");
      udp.print((int)emgFiltro); udp.print("\n");
