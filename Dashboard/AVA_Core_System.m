...
const int TAMANO_LOTE = 1; // Latencia CERO: envía cada muestra inmediatamente
int contador = 0;
char payload[512]; 
int payload_len = 0;
...
void setup() {
  ...
  Wire.begin(21, 22);
  Wire.setClock(100000); // Frecuencia I2C estándar para 100Hz
...
void loop() {
  ...
  while (samplesToRead--) {
    ...
    if (contador >= TAMANO_LOTE) {
      ...
      lotesEnviados++;
      if (lotesEnviados % 100 == 0) {  // Imprime 1 vez por segundo (100 lotes/seg a 100Hz con Lote=1)
